...
class CardMiddle extends \app\base\middle\BaseMiddle {
    ...
    protected function realInfo() {
        ...
        if(!$realInfo['status']) {
            return $this->stop('请先通过实名信息认证!', 302, '/pages/member/auth');
        }
    ...
    protected function post() {
        ...
        $this->realInfo();
        if(empty($this->data['realInfo'])) {
            if(!target('member/MemberReal')->real($this->params['user_id'], $this->params['real_name'], $this->params['real_idcard'])) {
                return $this->stop(target('member/MemberReal')->getError());
            }
            $name = $this->params['real_name'];
        }else {
        ...
        if (empty($receive)) {
            return $this->stop('该验证方式未绑定，请使用其他验证方式！', 302, '/pages/member/phone');
        }
