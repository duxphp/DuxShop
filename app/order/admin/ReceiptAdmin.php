...
class ReceiptAdmin extends \app\system\admin\SystemExtendAdmin {
    ...
    public function status() {
        ...
        if (!isPost()) {
            ...
            if (empty($orderPay)) {
                $orderPayNo = target('order/Order', 'service')->addPay($this->userInfo['user_id'], $info['order_id'], 'system');
                if (!$orderPayNo) {
                    $model->rollBack();
            ...
            if (!target('order/Order', 'service')->payOrder($orderPayNo, $money, '线下付款', $info['order_no'])) {
                $model->rollBack();
                $this->error(target('order/Order', 'service')->getError());
                return false;
            }
            ...
            if (!target('order/Order', 'service')->addLog($info['order_id'], '订单已完成收款', $remark, USER_ID)) {
                $model->rollBack();
                $this->error('订单日志记录失败!');
            }
