...
class OrderRefundModel extends SystemModel {
    ...
    protected function base($where) {
        ...
        $base = $this->table('order_refund(A)')
            ...
            ->join('order(D)', ['D.order_id', 'A.order_id']);
        $field = ['A.*', 'B.email(user_email)', 'B.tel(user_tel)', 'B.nickname(user_nickname)', 'B.avatar(user_avatar)', 'C.id(order_goods_id)', 'C.order_id', 'C.goods_name', 'C.goods_name', 'C.goods_image', 'C.goods_url', 'C.goods_qty', 'C.goods_price', 'C.price_total', 'C.extend', 'D.order_no', 'D.pay_price', 'D.delivery_price'];
