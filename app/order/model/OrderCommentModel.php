...
class OrderCommentModel extends SystemModel {
    ...
    protected function base($where) {
        ...
        $field = ['A.*', 'B.email(user_email)', 'B.tel(user_tel)', 'B.nickname(user_nickname)', 'B.avatar(user_avatar)', 'C.goods_name', 'C.goods_name', 'C.goods_image', 'C.goods_url', 'C.goods_qty','C.goods_price', 'C.price_total', 'C.extend', 'D.order_create_time', 'D.order_no'];
    ...
    public function loadList($where = array(), $limit = 0, $order = '') {
        ...
        foreach($list as $key => $vo) {
            ...
            $list[$key]['spec'] = unserialize($vo['spec']);
        }
        return $list;
    }
