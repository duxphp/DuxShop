...
class RefundMiddle extends \app\base\middle\BaseMiddle {
    ...
    protected function push() {
        ...
        if(empty($orderInfo['pay_currency']['exchange']) && empty($orderInfo['pay_currency']['append'])) {
            if (bccomp($price, 0, 2) !== 1) {
                return $this->stop('退款金额不正确错误!');
            }
        }
        if($type == 2) {
            if (bccomp($price, $this->data['orderPrice'], 2) === 1) {
                return $this->stop('退款金额不能大于订单金额!');
            }
        }else {
        ...
        if (!target('order/Order', 'service')->addLog($orderInfo['order_id'], '买家发起退款申请')) {
            target($this->_model)->rollBack();
    ...
    protected function cancel() {
        ...
        if (!target('order/Order', 'service')->addLog($info['order_id'], '买家已取消退款申请')) {
            target($this->_model)->rollBack();
            return $this->stop('订单日志记录失败!');
        }
        target($this->_model)->commit();
