...
class OrderMiddle extends \app\base\middle\BaseMiddle {
    ...
    protected function cancel() {
        ...
        if($orderInfo['pay_time'] && !$orderInfo['pay_status'] && $orderInfo['pay_time'] + 120 >= time()) {
            return $this->stop('订单支付状态更新中，请在2分钟后进行取消操作！');
        }

        if($orderInfo['status_data']['action'] <> 'pay' && $orderInfo['status_data']['action'] <> 'parcel') {
