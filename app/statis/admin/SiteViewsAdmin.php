...
class SiteViewsAdmin extends \app\system\admin\SystemExtendAdmin {
    ...
    public function index() {
        ...
        $mallPeople = target('statis/StatisViews')->query("select COUNT(DISTINCT user_id) as `count`  from `{pre}statis_views` where species = 'mall' and user_id > 0 and date >= " . $startTime . " and date <= " . $stopTime);
        ...
        $data = target('statis/StatisViews')->query("select date,sum(num) as view_sum  from `{pre}statis_views` where species = 'mall' and date >= " . $startTime . " and date <= " . $stopTime . " group by date");
        $listViewData = [];
        foreach ($data as $vo) {
            $listViewData[date('Y-m-d', strtotime($vo['date']))] += $vo['view_sum'];
        }
        ...
        $data = target('statis/StatisViews')->query("select date,COUNT(DISTINCT user_id) as `count`  from `{pre}statis_views` where species = 'mall' and user_id > 0 and date >= " . $startTime . " and date <= " . $stopTime . " group by date");
        ...
        $viewBarJs = target('tools/Echarts', 'service')->bar('mall-bar', $statsLabel, [
            [
                'name' => '会员访客',
                'data' => $peopleData,
            ],
        ...
        $this->assign('mallViewsJs', $viewBarJs);
