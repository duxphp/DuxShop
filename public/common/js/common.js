...
(function ($, owner) {
    ...
    owner.location = function ($el, config) {
        var defaultConfig = {
            url: rootUrl + '/c/tools/Area/index',
            province: '请选择省份',
            city: '请选择城市',
            region: '请选择区县',
            street: '请选择街道',
        ...
        var change = function (type) {
            ...
            switch (type) {
                case 'city':
                    data = {
                        'province': layout.find('[data-province]').val()
                    }
                    layout.find('[data-city]').html('<option value="">' + config.city + '</option>');
                    layout.find('[data-region]').html('<option value="">' + config.region + '</option>');
                    layout.find('[data-street]').html('<option value="">' + config.street + '</option>');
                    break;
                case 'region':
                    data = {
                        'province': layout.find('[data-province]').val(),
                        'city': layout.find('[data-city]').val()
                    }
                    layout.find('[data-region]').html('<option value="">' + config.region + '</option>');
                    layout.find('[data-street]').html('<option value="">' + config.street + '</option>');
                    break;
                case 'street':
                    data = {
                        'province': layout.find('[data-province]').val(),
                        'city': layout.find('[data-city]').val(),
                        'region': layout.find('[data-region]').val(),
                    }
                    layout.find('[data-street]').html('<option value="">' + config.street + '</option>');
                    break;
                default:
            }
            $.post(config.url, data, function (species) {
                var data = species[type];
                for (var i in data) {
                    var html = '<option value="' + data[i] + '">' + data[i] + '</option>';
                    layout.find('[data-' + type + ']').append(html);
                }
            }, 'json');
        }

        var init = function () {
            layout.find('[data-province]').html('<option value="">' + config.province + '</option>');
            layout.find('[data-city]').html('<option value="">' + config.city + '</option>');
            layout.find('[data-region]').html('<option value="">' + config.region + '</option>');
            layout.find('[data-street]').html('<option value="">' + config.street + '</option>');

            var preset = {
                province: layout.find('[data-province]').data('province'),
                city: layout.find('[data-city]').data('city'),
                region: layout.find('[data-region]').data('region'),
                street: layout.find('[data-street]').data('street')
            };
            $.post(config.url, preset, function (species) {
                for (var type in species) {
                    var data = species[type];
                    for (var i in data) {
                        if (preset[type] == data[i]) {
                            var html = '<option selected value="' + data[i] + '">' + data[i] + '</option>';
                        }else {
                            var html = '<option value="' + data[i] + '">' + data[i] + '</option>';
                        }
                        layout.find('[data-' + type + ']').append(html);
            ...
            layout.find('[data-province]').on('change', function () {
                change('city');
            });
            layout.find('[data-city]').on('change', function () {
                change('region');
            });
            layout.find('[data-region]').on('change', function () {
                change('street');
            });
        }
        init();
